...
import { usePropietarioProductoReglasController } from '../hooks/usePropietarioProductoReglasController'
// Reutilizar componentes del editor admin
import { ReglasCategoriasTable } from '@/features/adminMenuPlantillas/components/ReglasCategoriasTable'
import { ReglasIngredientesList } from '@/features/adminMenuPlantillas/components/ReglasIngredientesList'

type TabValue = 'categorias' | 'ingredientes'

export function ProductoReglasPage() {
  ...
  const { state, actions } = usePropietarioProductoReglasController()
        ...
        <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box sx={{ flex: 1 }}>
            <Typography variant="h4" sx={{ color: designTokens.colors.texto }}>
              Reglas de Personalización - Producto #{id}
            </Typography>
            <Typography variant="body2" sx={{ color: designTokens.colors.gris }}>
              Configura las reglas de personalización para este producto en tu menú general
            </Typography>
          </Box>
          ...
          <>
            ...
            {activeTab === 'categorias' && (
              <ReglasCategoriasTable
                categorias={state.reglas.categorias.map((c) => ({
                  id_producto: productoId, // Usar productoId para compatibilidad con componente
              ...
              <>
                {categoriaIds.length === 0 ? (
                  ...
                  <ReglasIngredientesList
                    ingredientes={state.reglas.ingredientes.map((i) => ({
                      id_producto: productoId, // Usar productoId para compatibilidad con componente
                      id_ingrediente: i.id_ingrediente,
                      ingrediente_nombre: i.ingrediente_nombre,
                      id_ingrediente_categoria: i.id_ingrediente_categoria,
                      categoria_nombre: i.categoria_nombre,
                      permite_cantidad: i.permite_cantidad,
...
export default ProductoReglasPage
