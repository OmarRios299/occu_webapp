/**
 * Hook controlador para editar reglas de un producto
 * Maneja estado, validaciones y guardado
 */
...
export function useAdminProductoReglasController(): {
  ...
  const actions: ControllerActions = {
    agregarCategoria: useCallback(
      (categoriaId: number) => {
        ...
        const categoria = reglas.catalogo_categorias.find(
          (c) => c.id === categoriaId
        )
        if (!categoria) return
        ...
        const nuevaCategoria: ReglaCategoria = {
          ...
          seleccion_minima: 0,
          seleccion_maxima: 1,
          orden: reglas.categorias.length > 0 
      ...
      (categoriaId: number, updates: Partial<ReglaCategoriaPayload>) => {
        ...
        setReglas({
          ...
          categorias: reglas.categorias.map((c) =>
            c.id_ingrediente_categoria === categoriaId
              ? { ...c, ...updates }
              : c
          ),
        })
    ...
    toggleIngrediente: useCallback(
      (ingredienteId: number, habilitado: boolean) => {
        ...
        const ingrediente = reglas.catalogo_ingredientes.find(
          (i) => i.id === ingredienteId
        )
        if (!ingrediente) return

        // Si se habilita y no existe, crear regla por defecto
        if (habilitado) {
          const existe = reglas.ingredientes.some(
          ...
          // Si se deshabilita, eliminar de la lista (pero mantener en catálogo)
          setReglas({
            ...reglas,
            ingredientes: reglas.ingredientes.filter(
              (i) => i.id_ingrediente !== ingredienteId
        ...
        setReglas({
          ...
          ingredientes: reglas.ingredientes.map((i) => {
            if (i.id_ingrediente === ingredienteId) {
              ...
              // Asegurar que si tipo_precio es 'fijo', siempre se apliquen las restricciones
    ...
    guardar: useCallback(async () => {
      ...
      try {
        ...
        // Mejorar mensaje de error para validaciones del backend
        let mensajeError = 'Error al guardar reglas'
        if (err instanceof HttpError && err.payload?.message) {
          mensajeError = err.payload.message
