...
final class PropietariosMenuGeneralRepository
{
    ...
    public function findProductos(array $filters, int $propietarioId): array
    {
        ...
        $sql = "SELECT 
            ...
            mp.imagen,
            COALESCE(pmp.habilitado, 0) AS estado_menu,
        ...
        if ($estadoFiltro !== null) {
            if ($estadoFiltro === 0) {
                $sql .= " AND (pmp.habilitado IS NULL OR pmp.habilitado = 0)";
            } else {
                $sql .= " AND pmp.habilitado = :estado_filtro";
        ...
        if ($estadoFiltro !== null) {
            if ($estadoFiltro === 0) {
                $countSql .= " AND (pmp.habilitado IS NULL OR pmp.habilitado = 0)";
            } else {
                $countSql .= " AND pmp.habilitado = :estado_filtro";
            }
        }
            ...
            try {
                $stmtTamanos = $this->pdo->prepare(
                    "SELECT 
                        ...
                        COALESCE(pmpt.habilitado, 0) AS estado_tamano,
            ...
            foreach ($productos as $prod) {
                ...
                if ($existe) {
                    ...
                    $stmtUpdate = $this->pdo->prepare(
                        ...
                        SET habilitado = :habilitado, estado = :estado, precio_base = :precio_base
                        WHERE id = :id"
                    );
                    $stmtUpdate->execute([
                        ':id' => $existe['id'],
                        ':estado' => $estado === 2 ? 2 : 0,
                        ':habilitado' => $estado === 2 ? 0 : max(0, min(1, $estado)),
                    ...
                    $stmtInsert = $this->pdo->prepare(
                        ...
                        (id_propietario, id_producto, precio_base, habilitado, estado, id_alta, fecha_alta)
                        VALUES (:propietario_id, :producto_id, :precio_base, :habilitado, :estado, :id_alta, :fecha_alta)"
                    );
                    $stmtInsert->execute([
                        ':propietario_id' => $propietarioId,
                        ':producto_id' => $productoId,
                    ...
                    foreach ($prod['tamanos'] as $tamano) {
                        ...
                        if ($existeTamano) {
                            $stmtUpdateTamano = $this->pdo->prepare(
                                ...
                                SET precio = :precio, habilitado = :habilitado, estado = :estado
                                WHERE id = :id"
                            );
                            $stmtUpdateTamano->execute([
                                ':id' => $existeTamano['id'],
                                ':precio' => $precio,
                                ':habilitado' => $estadoTamano === 2 ? 0 : max(0, min(1, $estadoTamano)),
                                ':estado' => $estadoTamano === 2 ? 2 : 0,
                            ]);
                        } else {
                            $stmtInsertTamano = $this->pdo->prepare(
                                "INSERT INTO propietarios_menu_productos_tamanos 
                                (id_propietario_menu_producto, id_tamano, precio, habilitado, estado)
                                VALUES (:id_producto, :id_tamano, :precio, :habilitado, :estado)"
                            );
